...
if os.path.exists(os.path.join(path_models, "sampling_detector.json")):
    # Load sampling from disk
    with open(os.path.join(path_models, "sampling_detector.json"), "r") as json_reader:
    ...
    for i, fold in enumerate(subsets):
        ...
        sampling_dict["cv_" + str(i)] = {"x_train": x_train.tolist(),
                                         ...
                                         "x_val": x_val.tolist(),
                                         "y_val": y_val.tolist()}
    with open(os.path.join(path_models, "sampling_detector.json"), "w") as file:
...
for i, fold in enumerate(subsets):
    ...
    # Define callbacks
    cb_mc = ModelCheckpoint(os.path.join(path_detect, "cv_" + str(i) + \
                                         ".model.best.hdf5"),
                            monitor="val_loss", verbose=1,
                            save_best_only=True, mode="min")
    cb_cl = CSVLogger(os.path.join(path_detect, "cv_" + str(i) + ".logs.csv"),
